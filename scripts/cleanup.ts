...
import { deleteExpiredExports } from "@/services/exports";
...
export async function cleanup(): Promise<void> {
  ...
  const expiredExports = await deleteExpiredExports();
  await db.query("DELETE FROM jobs WHERE status IN ('succeeded', 'dead') AND created_at < now() - interval '30 days'");
  await db.query("DELETE FROM idempotency_keys WHERE created_at < now() - interval '30 days'");

  log.info("cleanup finished", { abandonedPhotosRemoved: removed.length, expiredExports });
}
