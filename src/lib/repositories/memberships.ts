...
export async function createMembership(db: Queryable, input: CreateMembershipInput): Promise<MembershipRow> {
  const row = await queryOne<MembershipRow>(
    db,
    `INSERT INTO memberships (tenant_id, email, friend_name, role, platform_user_id)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING *`,
    [input.tenantId, input.email, input.friendName, input.role ?? "friend", input.platformUserId ?? null]
  );
